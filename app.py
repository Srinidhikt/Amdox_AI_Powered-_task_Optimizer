...
def dashboard():
    ...
    # Load employee moods from employee_mood_history.csv
    mood_history = load_csv(EMP_MOOD_HISTORY_CSV)
    emp_moods = []
    ...
    if request.method == "POST":
        ...
        # Save to employee_mood_history.csv
        save_to_csv(timestamp, empid, empname, sentence, emotions)

        # Save latest emotions to mood_history.csv (for HR reference)
...
def hr_dashboard():
    ...
    # Collect full mood history per employee
    full_history = {}
    alerts = {}
